...
app.get("/cafes/:id", (req, res) => {
    ...
    else res.status(404).send({ message: "no existe un cafe con esta id" })
})
...
app.post("/cafes", (req, res) => {
    ...
    if (existeUncafeConEseId) res.status(400).send({ message: "ya esta un cafe con ese id" })
    else {
        cafes.push(cafe)
...
app.put("/cafes/:id", (req, res) => {
    ...
    if (id != cafe.id)
        return res
            .status(400)
            .send({
                message: "id del parámetro no coincide con id del café",
            });
